...
- [ ] Zero-code A/B: raise the live pythonw processes to Normal and read the fps log for a few minutes.
- [ ] Instrument `App.tick`: time each step (presses, lock, alerts, tiles, scene render, flush) and log any step over 100 ms with its name; log the priority class at start. Run on the deck and read the result.
- [ ] Fix the named cause (if priority: `install_task.ps1` sets task priority 4 = Normal and Wes re-registers from his terminal; otherwise whatever the log names). Retest with the fps log.
- [ ] CPU clock: `sources/sysmon.py` reads the PDH counters once per second (opened once, formatted as double); `ghz` = performance% x nominal MHz / 1000; the tile's bottom line shows `x.x GHz`; zoom unchanged (RAM already there). Tests with a fake reader.
- [ ] Housekeeping: `git branch -d` the two merged branches; allow rules to `~/.claude/settings.json` via the update-config skill, project file emptied, config repo committed.
...
Status at hand-off, 2026-09-06 20:26: every gate above is closed and `main` is pushed (`origin/main` = `main` = `0ca19df` plus this checkpoint commit). The scheduled task is running from the main checkout with the six-scene rotation at 14 fps; 95 tests pass. Housekeeping left for Wes: close the desktop-app tabs for `sweet-mclean-8f59b9` and `stoic-roentgen-a34d62`, then delete the two empty folders under `.claude\worktrees\`; the current session worktree `exciting-lehmann-b57392` (branch fast-forwarded into `main`) can go the same way once its tab is closed. Sixteen read-only PowerShell allow rules live in `.claude/settings.local.json` (gitignored; Wes may move them to user scope). Optional follow-ups on request: more scenes (strawman plus numbered questions with recommended defaults first); the CPU clock via the PDH counter `% Processor Performance` (Phase 1 follow-up); an fps log check on the matrix and aquarium scenes (only tokyo was measured).
