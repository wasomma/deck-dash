...
- [ ] Overnight: `grep "slow tick" logs/deckdash.log` in the main checkout after a night of ambient rotation; expect none outside heavy load. Interim check 22:34 (no night yet): since the 22:19:44 restart the log holds 10 min of board mode without a slow-step warning, then `weather` at 13.9 fps with 0 slow ticks in both reported minutes (flush avg 41 ms, max 85 and 60 ms), ended by a key press at 22:31:48. Nothing more to compare until the morning.
...
- [ ] `pyproject.toml` (setuptools backend, name `deck-dash`, runtime dependencies = `requirements.txt` minus pytest, plus `pystray`): `[project.scripts] deckdash = "deckdash.main:main"` and `[project.gui-scripts] deckdashw = "deckdash.main:main"`. Editable install into the main-checkout venv (`pip install -e .`; pip fetches setuptools as the build backend, a one-time download). `python -m deckdash` keeps working. `install_task.ps1` runs `.venv\Scripts\deckdashw.exe`; the existing registration keeps working until Wes re-registers from his own terminal.
- [ ] `deckdash/control.py`: a daemon thread serves the Windows named pipe `\\.\pipe\deckdash` with stdlib `multiprocessing.connection` (`family="AF_PIPE"`, no port, same user only). One request tuple per connection, one reply dict (`ok`, `error`, payload). Commands go into `App.commands` (`queue.Queue`) and `_tick` drains it first (a "commands" mark). Commands: `status` (mode, scene, uptime, paused, locked, brightness, the last fps-line values, slow ticks since start, per-source last-fetch age and last error), `wake`, `scene NAME`, `next`, `pause`, `resume`, `brightness N|auto`, `toast KIND TITLE DETAIL` (a test toast through the normal toast path), `reload` (re-read config, rebuild slots, scene list, brightness and idle settings; sources untouched), `quit` (exit 0).
- [ ] CLI: `deckdash ctl <command> [args]`; `argv[0] == "ctl"` dispatches to `deckdash/ctl.py` before argparse and prints the reply (`--json` for the raw dict). Two client-side commands: `ctl restart` runs `tools\install_task.ps1 -Stop` then `-Start` from the CLI process; `ctl open` opens the dashboard (7b; in 7a it prints the URL).
- [ ] Pause (`pause` / `resume` from ctl, tray or dashboard): brightness 0, one tick per second, no rendering, sources keep polling, toasts wait as they do while locked; resume restores brightness and invalidates, so the board is current at once.
- [ ] Second launch while the mutex is held (no `--sim`): open the dashboard and exit 0 (7b); until 7b lands it keeps exiting silently.
- [ ] `deckdash/tray.py` (pystray, `icon.run_detached()` so the loop keeps the main thread; import-guarded, a missing package logs one line and the app runs without a tray). Icon: a 3x5 dot grid drawn with Pillow, green normally, grey while paused or locked, amber while a badge is pending. Menu: Wake board; Scene submenu (six scenes plus Next); Pause / Resume; Brightness (Auto, 100, 50, 30); Open dashboard; Open log (`os.startfile` on `logs\deckdash.log`); Edit config (`config.local.toml`, created empty if missing); Recalibrate gap (detached PowerShell: `install_task.ps1 -Stop`, `tools\calibrate.py`, `-Start`); Restart (detached PowerShell: `-Stop`, `-Start`); Quit. Left click = Open dashboard. Menu actions post to `App.commands` directly. `[ui] tray = false` in `config.toml` turns it off.
- [ ] Verify on the hardware: the icon appears under the task-launched process (interactive per-user logon task); pause/resume, a scene switch and a test toast from the tray and from `deckdash ctl`; the ambient fps line unchanged (13.9 fps, 0 slow ticks) with the pipe thread and the tray running.
- [ ] Tests: pipe round trip against a running `App` on `SimDeck`, every command's effect, pause semantics, reload rebuilding slots, tray icon rendering, `ctl` argument parsing.

### 7b: dashboard page and window (one session)
- [ ] `deckdash/dashboard.py`: stdlib `ThreadingHTTPServer` on `127.0.0.1:8770` (`[ui] dashboard_port`) in a daemon thread. Routes: `/` serves `deckdash/ui/index.html` (one file, plain HTML/JS/CSS, dark, no CDN); `/api/status` = the `status` command; `/api/frame.png` = the device's last fifteen key images composed on a 5x3 grid with `gap_px` between keys, PNG, cached for 200 ms; `/api/cmd` POST `{cmd, args}` = the control commands plus `restart` and `open`; `/api/config` GET = the effective values of the tunables, POST = validate, `config.write_local`, `reload`, and `restart_required: true` in the reply when feeds or repos changed.
- [ ] Page panels: live preview (5 fps `<img>` refresh); status (mode, scene, fps, flush avg/max, slow ticks, per-source age and error, brightness, lock); controls (wake, scene buttons, next, pause/resume, brightness, test toast, restart, quit); settings form for the tunables list: layout (15 selects from the tile registry plus blank), overlays, scene order and minutes and fps, idle minutes, day and night brightness with the night window, `slow_step_ms`, news feeds, CI repos. Writes go to `config.local.toml` only; `config.toml` stays canonical; hosts and addresses stay file-edited.
- [ ] Restart from the page: the server spawns a detached PowerShell (`install_task.ps1 -Stop`, then `-Start`) that survives the app's exit (the job's silent-breakaway flag allows it).
- [ ] Window: `msedge --app=http://127.0.0.1:8770` (Edge at its standard Program Files path, else `webbrowser.open`); the tray's Open dashboard, `ctl open` and a second launch all use it.
- [ ] Acceptance on the deck: tokyo for two minutes with the window open and the preview streaming = 13.9 fps, 0 slow ticks, flush max within 10 ms of the Phase 6 baseline (66-70 ms).
- [ ] README: a "Using it" section (command, tray, dashboard, ctl).
...
Gate before 7a: Wes approves `pip install pystray` plus the one-time setuptools fetch for the editable install, and says "go".
...
Status at hand-off, 2026-09-06 22:35: Phase 6 is code-complete and live, including the board-mode stall fix (power throttling opt-out). `main` is level with `origin/main` at 1e08e25 (Phase 6 pushed); this session works on `claude/blissful-galileo-5ec655`, which carries only PHASES/README updates and fast-forwards onto `main`; push on "push it". The scheduled task (re-registered by Wes at priority 5) runs from main with the slow-step log; 100 tests pass. Open: the overnight slow-tick grep (interim at 22:34: 12 clean minutes, 0 slow ticks) and, on "do it", the README refresh (drop the Phase 3 wording, mention the PDH CPU clock, the `-Stop`/`-Start` switches, the startup normalization line and the slow-tick log). The previous session's worktree `exciting-lehmann-b57392` is fully merged; archive its sidebar session, then `git worktree remove --force` it. Phase 7 (deck-dash as an app: command, named-pipe control, tray, local dashboard, Edge app-mode window) is specified above with all nine defaults accepted; 7a starts on "go" once Wes approves the pystray and setuptools downloads.
